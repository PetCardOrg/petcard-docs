...
Collection e environment do Postman para explorar a **API REST do PetCard**, cobrindo os **53 endpoints** em 15 grupos. Gerados a partir do contrato **OpenAPI** da API (PC-097).

## Arquivos

| Arquivo | Descrição |
| --- | --- |
| `petcard-api.postman_collection.json` | Collection (schema Postman v2.1) — 15 pastas por domínio, 53 requests, com corpo de exemplo e exemplo de resposta em cada uma. |
...
- A collection define **Bearer `{{access_token}}`** no nível raiz; os 49 endpoints protegidos herdam.
- São **públicos** (`noauth`): `POST /auth/login`, `POST /auth/register`, `POST /auth/veterinario/login` e `GET /cards/:token` (carteira pública via QR).
